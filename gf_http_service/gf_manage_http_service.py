...
def get_http_service(module, url, auth, headers):
    ...
    try:
        ...
        module.fail_json(msg=f"Failed to get HTTP service properties. Error: {str(e)}")

def ensure_http_service(module, url, auth, headers, desired_access_logging, desired_sso):
    """
    ...
    if current_http_service['access_logging_enabled'] != desired_access_logging:
        module.debug(msg=f"access_logging_enabled: current={current_http_service['access_logging_enabled']}, desired={desired_access_logging}")
        changed = True
    if current_http_service['sso_enabled'] != desired_sso:
        module.debug(msg=f"sso_enabled: current={current_http_service['sso_enabled']}, desired={desired_sso}")
        changed = True
    ...
    if changed:
        ...
        response = requests.post(url, json=payload, auth=auth, headers=headers, verify=module.params['validate_certs'])
        if response.status_code == 200:
            module.debug(msg=f"Updated HTTP service properties successfully.")
        else:
            module.fail_json(msg=f"Failed to update HTTP service properties. Status code: {response.status_code}, Response: {response.text}")
...
def main():
    ...
    changed = ensure_http_service(module, url, auth, headers, access_logging_enabled, sso_enabled)
